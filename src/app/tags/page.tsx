  ...
  return (
      <div className="container flex flex-col justify-center gap-12 px-4 py-16 w-full">
        ...
        {currentTagData && <div>
          ...
          <p>Namespace: {currentTagData.namespace.map((item, i) => { return (
            <span className="mr-2" key={i}>{item}</span>
          )})}</p>
          ...
          <br />
          <h3>Hierarchy</h3>
          <p>Allowed parrents: {currentTagData.hierarchy.parentTags.map((item, i) => { return (
            <span className="mr-2" key={i}>{item}</span>
          )})}</p>
          <p>Allowed childs: {currentTagData.hierarchy.childTags.map((item, i) => { return (
            <span className="mr-2" key={i}>{item}</span>
          )})}</p>
          <br />
          <p>Intended for: {currentTagData.roles.intendedFor.map((item, i) => { return (
            <span className="mr-2" key={i}>{item}</span>
          )})}</p>
          <p>Misuse prevention: {currentTagData.roles.misusePrevention}</p>
          <br />
          <h3>ValidationTips</h3>
          {currentTagData.validationTips.map((item, i) => { return (
            <p className="mr-2" key={i}>{item}</p>
          )})}
        </div>}
      </div>
  );
